...
class RegisterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        registerRepeatPassword = findViewById(R.id.resgisterRepeatPassword)
        ...
        auth.createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener(this) {task ->
                if (task.isSuccessful){
                    ...
                    //El intento de cambiar de pantalla también se puede hacer así
                    //startActivity(Intent(this, MainActivity::class.java))
                    //Finalizo esta actividad para que no ocupe espacio
                    finish()
                }else{
                    Toast.makeText(applicationContext, "Error registrándose", Toast.LENGTH_LONG).show()
                }
            }
    }

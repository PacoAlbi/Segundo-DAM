...
public class Accesos {
    ...
    public static void LeerFichSecuencial() throws IOException {
        File fichero = new File("NUEVODIR/FICHERO1.TXT");
        FileReader fic = new FileReader(fichero);
        char[] letras = new char[(int) fichero.length()];
        while ((fic.read(letras)) != -1) { //se va leyendo un carÃ¡cter hasta que lea -1
            System.out.println(letras); //cast a char
        }
        fic.close(); //cerrar fichero
    }

    public static void EscribirFichAleatorio() throws IOException {
